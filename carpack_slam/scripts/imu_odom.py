...
frame_name  = rospy.get_param('~frame_name',"odom")
child_frame_name = rospy.get_param('~child_frame_name',"base_footprint")
pub_name    = rospy.get_param('~pub_name',"odom_topic")
sub_name    = rospy.get_param('~sub_name',"imu_data")
sub2_name   = rospy.get_param('~sub2_name',"joint_states")
start_x     = rospy.get_param("~start_x",-0.5)
start_y     = rospy.get_param("~start_y",0)
start_z     = rospy.get_param("~start_z",0)
class odom_class:
    odom_topic = Odometry()
    ...
    def __init__(self):
        ...
        # 定义时间差
        rate = rospy.Rate(20)
        #self.now_time_imu = rospy.Time.now().to_sec()
        #self.old_time_imu = rospy.Time.now().to_sec()
    ...
    def callback_imu(self, data):
        # 计算时间差
        #self.now_time_imu = rospy.Time.now().to_sec()
        #add_time_imu = self.now_time_imu - self.old_time_imu
        #self.old_time_imu = self.now_time_imu
        # 正交分解 偏航角（必须）
    ...
    def callback_join(self, data):
        ...
        #lfw_topic = data.name.index('left_front_axle')
        #rfw_topic = data.name.index('right_front_axle')
        ...
        if True :
            ...
            #velocity2 = data.velocity[lfw_topic] + data.velocity[rfw_topic]
            #velocity = velocity2 / 2 + velocity1 / 2
            # 平均路程
            position = velocity * add_time_joint 
        else:
            position = ((data.position[lrw_topic] + data.position[rrw_topic])/2) - self.old_position
            velocity = position / add_time_joint
        ...
        #print(velocity)
        #print(position)
        #print(add_time_joint)
        #print(self.yaw_angle[1])
        #print(self.yaw_angle[2])
        #print(self.odom_topic.twist.twist.linear.x)
        #print(self.odom_topic.pose.pose.position.x)
        #print("-----")
